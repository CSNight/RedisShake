...
func (s2s *Standalone2StandaloneCase) Before() error {
	...
	if err := deploy.Deploy(deploy.StandaloneScript, s2s.SourcePort, deploy.CmdStart, 0); err != nil {
		return fmt.Errorf("deploy source redis failed: %v", err)
	}

	// deploy target redis with given port
	if err := deploy.Deploy(deploy.StandaloneScript, s2s.TargetPort, deploy.CmdStart, 0); err != nil {
		return fmt.Errorf("deploy source redis failed: %v", err)
	}
...
func (s2s *Standalone2StandaloneCase) Run() error {
	// build client
	shakeUtils.OpenRedisConn([]string{fmt.Sprintf(":%d", s2s.SourcePort)}, "auth", "", false, false)
	shakeUtils.OpenRedisConn([]string{fmt.Sprintf(":%d", s2s.TargetPort)}, "auth", "", false, false)
	return nil
}
